(function ($, scope, undefined) {
    ...
    NextendSmartSliderAbstract.prototype.__postInit = function (id, sliderElement, parameters) {
        ...
        if (this.parameters.lightbox.length > 0) {
            var lightboxes = [];
            for (var i = 0; i < this.parameters.lightbox.length; i++) {
                lightboxes.push($("<a href=\"" + this.parameters.lightbox[i] + "\" n2-lightbox=\"\" data-litebox-group=\"" + this.id + "\" style=\"display:none;\"></a>").on('click', function (e) {
                    e.stopPropagation();
                }).appendTo(this.slides.eq(i)));
            }
            ...
            this.slides.on('click', $.proxy(function () {
                lightboxes[this.currentSlideIndex].trigger('click');
            }, this));
        }
